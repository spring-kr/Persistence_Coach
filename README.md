...
[사용자 로그 / 발화 입력]
    ...
    ▼
┌──────────────────────────────────────────────┐
│  결정적 코어 (Deterministic Core: Python/JS)   │
│  - 5-State FSM (ACTIVE/DECLINING/PARALLEL/..)│
│  - Rule R15 선행 추세 감지기                   │
│  - 임상 안전 가드레일 인터셉트                  │
└──────────────────────────────────────────────┘
   │  (확정된 상태, DO/NEVER 정책, 트리거 태그)
   ▼
┌──────────────────────────────────────────────┐
│  표현 레이어 (LLM / Local Dialogue Engine)    │
│  - 코어의 판단을 왜곡하지 않고 담백하게 전달      │
│  - 죄책감 유발 금지, 일상어 변환                │
└──────────────────────────────────────────────┘
